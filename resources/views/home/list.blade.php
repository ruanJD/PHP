...
@section('info')
    <title>{{$field->cate_name}}-{{Config::get('cofweb.web_title')}}</title>
...
<article class="blogs">
    ...
    <div class="newblog left">
        @foreach($data as $d)
        <h2>{{$d->art_title}}</h2>
        <p class="dateview"><span>发布时间：{{date('Y-m-d / H:m',$d->art_time)}}</span><span>作者：{{$d->art_editor}}</span><span>分类：[<a href="{{url('cate/'.$field->cate_id)}}">{{$field->cate_name}}</a>]</span></p>
